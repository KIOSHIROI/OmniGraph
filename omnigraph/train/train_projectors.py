...
def load_region_pairs(region_path: str) -> List[Tuple[int, str]]:
    ...
    返回: [(image_id, phrase), ...]
    """
    data = json.load(open(region_path, "r", encoding="utf-8"))
    ...
    for item in data:
        ...
        for r in regions:
            ...
            phrase = r.get("phrase", "")
            if phrase:
                pairs.append((image_id, str(phrase).strip()))
...
class VGGraphRegionTextDataset(torch.utils.data.Dataset):
    ...
    输出仍为: {id, graph_data, text, answer}
    """
    ...
    def __init__(
        ...
        # 要求 VGSceneGraphDataset 暴露 raw json list 为 .items 或 .scene_graphs
        raw_items = None
        ...
        # raw_items 预期为 list，每个 item 含 image_id
        ...
        self.samples: List[Tuple[int, str]] = [
            (iid, phr) for (iid, phr) in region_pairs if iid in self.image2idx
        ]
...
class ProjectorPL(pl.LightningModule):
    """
    训练：vg_adapter + graph_qformer + gl_projector
    冻结：LLM + graphgpt (+ vision branch)
    """
    ...
    def training_step(self, batch: Dict[str, Any], batch_idx: int):
        ...
        if isinstance(debug, dict) and "graph_graph_node_reps_nan" in debug:
            self.log(
                "dbg_graph_node_nan",
                float(debug["graph_graph_node_reps_nan"]),
                prog_bar=False,
                on_step=True,
            )
...
def main():
    ...
    ap.add_argument("--freeze_graph_qformer", action="store_true", help="only train vg_adapter + gl_projector")
    args = ap.parse_args()
    ...
    pl_model = ProjectorPL(
        ...
        freeze_graph_qformer=args.freeze_graph_qformer,
    )

    # 5) callbacks
    ckpt = ModelCheckpoint(
        dirpath=args.save_dir,
        filename="projector-{epoch:02d}-{train_loss:.3f}",
        save_top_k=1,
        monitor="train_loss",
        mode="min",
    )
    ...
    if use_gpu:
        if int(args.gpu) >= torch.cuda.device_count():
            raise ValueError(
                f"--gpu={args.gpu} is out of range (device_count={torch.cuda.device_count()})."
            )
    ...
    # 6) save full model state (包含 projector / adapter / qformer)
    out_path = Path(args.save_dir) / "omnigraph_projector_stage.pt"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(pl_model.model.state_dict(), str(out_path))
